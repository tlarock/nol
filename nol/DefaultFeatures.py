...
def calculate_features(self, order='linear'):
    ...
    # FEATURE 5 number of edges observed by probing other nodes
    if self.rewardCounts.max() > 0:
        number_probed_edges = (self.rewardCounts - self.rewardCounts.min()) / (self.rewardCounts.max() - self.rewardCounts.min())
    else:
        number_probed_edges = self.rewardCounts

    if self.attribute_dict is None:
        new_feature_matrix = np.column_stack((deg, cc_array, connected_component_sizes, frac_probed_neighbors, number_probed_edges))
    else:
        num_target_neighbors = number_target_neighbors(self)
        #frac_target_neighbors = num_target_neighbors / self.D
        new_feature_matrix = np.column_stack((deg, cc_array, connected_component_sizes, frac_probed_neighbors, number_probed_edges, num_target_neighbors))
...
def update_features(self, probed_node, order='linear'):
        ...
        if self.attribute_dict is not None:
            number_target_neighbors = np.concatenate((self.F[:,5], np.zeros(extension_length)))
        ...
        if self.attribute_dict is not None:
            ...
            new_feature_matrix = np.column_stack((deg, cc_array, component_size_array, frac_probed_array, number_probed_edges, number_target_neighbors))
        else:
            new_feature_matrix = np.column_stack((deg, cc_array, component_size_array, frac_probed_array, number_probed_edges))
    ...
    for neighbor in self.sample_graph_adjlist[probed_node]:
        ...
        if self.attribute_dict is not None:
            if 0 in self.attribute_dict[probed_node]:
                new_feature_matrix[neighbor_row, 5] += 1
