...
do
	...
	do
			...
			python3 $program -m $model --attr $attribute_file --target $target_attribute --seeds $seeds -i $base_input -s $sample_para -o $output_folder -n $networks -iter $iterations -b $budget --alpha $a  --feats $featuretype --reward $rewardfunction --save_gap $savegap -p $p --decay $decay --burn $burnin --sampling-method $sampling_method &
